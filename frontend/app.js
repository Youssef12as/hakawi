...
/**
 * Split text into chunks for TTS playback (around 40 words each).
 * Looks for commas near the 40-word mark to split naturally.
 */
function splitIntoBreathGroups(text) {
    ...
    while (currentChunkStart < words.length) {
        ...
        // Look for a comma in the window [targetEnd - 15, targetEnd + 15]
        ...
        for (let i = windowStart; i <= windowEnd; i++) {
            if (words[i].endsWith('،') || words[i].endsWith(',')) {
                // Pick the comma closest to the targetEnd
        ...
        if (bestCommaIndex !== -1) {
            splitWordIndex = bestCommaIndex + 1; // Split AFTER the word with the comma
        }
...
/**
 * Main pipeline: fire all TTS requests in parallel, play in order.
 * Chunk 1 plays immediately once ready; chunk 2 generates simultaneously.
 */
async function playSentencePipeline(fullText) {
    ...
    const audioPromises = sentences.map(s => fetchSentenceAudio(s));
    ...
    for (let i = 0; i < sentences.length; i++) {
        ...
        // Wait only for THIS chunk's audio (the other is already in-flight)
