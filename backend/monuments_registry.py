"""
Monument registry — the single source of truth for the map.

Each monument is keyed by the EXACT name stored in the RAG chunks
(`backend/data/rag/embeddings.json` -> chunk["monument"]), so the RAG
service can filter chunks to the selected monument with a simple string
match.

Governorates currently exposed on the map:
    aswan, luxor, cairo, alexandria

`character_name` is the TTS voice identifier.  For now every monument
uses the "am-othman" voice/video; the team will record per-character
voices and drop matching video files into `frontend/public/character/`
later — only this registry needs updating then.
"""
